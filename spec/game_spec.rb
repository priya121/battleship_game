...
describe Game do 
  ...
  let(:coordinates) {['∙','◦','∙','∙']}
  let(:grid) {Grid.new(["E","E","E","E"],ROWS,COLUMNS).place_ship(1)}
  ...
  describe 'two by two grid' do
    ...
    it 'creates the table with coordinates' do 
      Game.new(input,output,ROWS,COLUMNS,grid).display_table(coordinates)
    ...
    it 'creates a table with y axis letters' do 
      Game.new(input,output,ROWS,COLUMNS,grid).display_table(['∙'])
      expect(table[1]).to include("A")
    end
    ...
    it 'displays an updated grid once a player has made a guess' do 
      grid = Grid.new(["E","E","E","E"],ROWS,COLUMNS).place_ship(1)
      Game.new(input,output,ROWS,COLUMNS,grid).start
      expect(table[1]).to include("1","2")
      expect(table[2]).to include("∙")
    end
  ...
  describe 'three by three grid' do 
    ...
    it 'displays an empty 3 by 3 grid' do 
      grid = Grid.new(["E","E","E","E","E","E","E","E","E"],rows,columns).place_ship(1)
      input = StringIO.new("A1\nA2\n")
      Game.new(input,output,rows,columns,grid).display_table(grid)
  ...
  describe "a 3 by 3 game which doesn't exit till all hits made" do
    let(:rows) {['A','B','C']}
    let(:columns) {['1','2','3']}
    let(:grid) {Grid.new(["E","E","E","E","E","E","E","E","E"],rows,columns)}
    ...
    it 'tells the user they sunk a battleship' do 
      ...
      grid_with_ship = Grid.new(["E","E","E","E","E","E","E","E","E"],rows,columns).place_ship(4)
      Game.new(input,output,rows,columns,grid_with_ship).start
      expect(output.string).to include("You sunk 1 battleships.")
    end

    it 'tells the user when they have sunk a destroyer' do 
      input = StringIO.new("A2\nA3\n")
      grid_with_ship = Grid.new(["E","E","E","E","E","E","E","E","E"],rows,columns)
